...
- Bars: `7 3 * * 2-6` (03:07 UTC ≈ 23:07 ET the evening of the session, 10:07 Bangkok). Runs `ingest:bars --days=7` then `snapshot:portfolio`. `ingest:bars` also scores `fundamental_inflection_v1`. Tue–Sat, because the run lands the calendar day after the session it closes out.
- **A green run is not evidence the store is current.** Twice in September 2026, at the earlier 18:37 ET slot, Yahoo served a window ending at the *previous* session: every symbol reported "4 bars via yahoo", the snapshot keyed the day before, and the book sat a day behind with nothing failing. Moving to 23:07 ET gives the vendor seven hours after the close instead of two and a half.
- Bars retry: `7 5 * * 2-6` (05:07 UTC, 12:07 Bangkok) runs `bars:freshness` first and re-ingests **only** when the store still does not reach `lastCompletedCashSession()` — the honest check being SPY's calendar plus every open holding's own latest bar, so one lagging name is enough. Both passes ingest on a bad morning; a good morning costs one query. An unset or failed check counts as stale, because a redundant pass costs two minutes and a skipped one costs the briefing.
- A retry that is *still* behind logs the verdict and does not fail: the weekday-only session clock cannot distinguish a lagging vendor from a US market holiday, and roughly nine red runs a year would train the operator to ignore the signal.
- Fundamentals: `37 8 * * 0` (Sunday 08:37 UTC). Runs `ingest:fundamentals` (SEC companyfacts, Yahoo hole-fill), which also scores.
- **All three are deliberately off the hour.** GitHub creates scheduled runs best-effort and is worst at `:00`. On `0 22`, 11 measured runs landed a median **176 minutes** late and one **474 minutes** late, with **zero runner wait** — the delay was entirely in GitHub's scheduler, not in capacity, so a bigger runner would not have helped. Nothing was ever skipped. Lateness is a freshness problem rather than a correctness one, because snapshots are keyed on the cash session: a run at 05:54 the next morning writes the same rows. If punctuality ever does matter, drive `workflow_dispatch` from an external scheduler rather than trusting `schedule`.
