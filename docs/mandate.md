...
10. **Factor concentration (correlation-aware)** — theme labels are not diversification. AI infra, data-center power/cooling, AI-linked energy, GPU clouds, and much of robotics/defence electronics all load on the same factor: **hyperscaler capex sentiment**. Each name has an **explicit weighted factor map**; unknown names are unclassified and flagged, not silently treated as AI-capex. Cap the weighted AI-capex exposure as one position-like risk **vs NAV (cash included)**, aligned with the theme cap, and maintain a standing "hyperscaler capex guidance −20%" stress (haircut × mapped weight) before adding correlated risk. Genuine diversifiers have zero mapped AI-capex weight (uranium fuel cycle, appropriations-driven defence primes, most backlog construction) — not a third cooling vendor. Cash is a diversifier versus this factor; the kill-switch (rule 8) stays on deployed capital.
